...
public class SparkseeGraph implements Graph, SparkseeGraphMBean {
	...
	private SparkseeGraph(final Configuration configuration) {
		...
		if (!dbFile.getParentFile().exists()
			...
			throw new InvalidParameterException(String.format(
					"Unable to create directory %s.", dbFile.getParent()));
		}
		...
		try {
			...
			sparksee = new com.sparsity.sparksee.gdb.Sparksee(
					new com.sparsity.sparksee.gdb.SparkseeConfig());
			if (!dbFile.exists()) {
	 ...
	 * @param <G>
	 *            the {@link com.tinkerpop.gremlin.structure.Graph} instance
	...
	public static <G extends Graph> G open(final Configuration configuration) {
		...
		if (!configuration.containsKey(DB_PARAMETER)) {
			throw new IllegalArgumentException(String.format(
					"Sparksee configuration requires %s to be set",
					DB_PARAMETER));
		}
			...
			public void run() {
				...
				try {
					...
					ObjectName name = new ObjectName(
							"com.tinkerpop.gremlin.sparksee.structure:type=SparkseeGraph");
	...
	public String compute(String algebra, Map<String, Object> params) {
		...
		//((SparkseeTransaction) this.tx()).commit(transactionId, timestamp);Mike
	...
	public Vertex addVertex(final Object... keyValues) {
		...
		final String label = ElementHelper.getLabelValue(keyValues).orElse(
	...
	public Vertex v(final Object id) {
		...
		try {
			...
			com.sparsity.sparksee.gdb.Graph rawGraph = transaction
					.getRawGraph();
			...
			return new SparkseeVertex(longId, rawGraph.getType(type).getName(),
					this);
		} catch (Exception e) {
			throw Graph.Exceptions.elementNotFound(Vertex.class, id);
		}
	...
	public Edge e(final Object id) {
		...
		try {
			...
			return new SparkseeEdge(longId, rawGraph.getType(type).getName(),
					this);
		} catch (Exception e) {
			throw Graph.Exceptions.elementNotFound(Vertex.class, id);
		}
	...
	public Iterator<SparkseeVertex> vertices() {
		...
		for (Integer type : typeList) {
			...
			com.sparsity.sparksee.gdb.ObjectsIterator objsIterator = objs
					.iterator();
			while (objsIterator.hasNext()) {
				long oid = objsIterator.next();
				list.add(new SparkseeVertex(oid, rawGraph.getType(type)
						.getName(), this));
			}
			objsIterator.close();
			objs.close();
	...
	public Iterator<SparkseeEdge> edges() {
		...
		for (Integer type : typeList) {
			...
			while (objsIterator.hasNext()) {
				long oid = objsIterator.next();
				list.add(new SparkseeEdge(oid,
						rawGraph.getType(type).getName(), this));
			}
			objsIterator.close();
			objs.close();
		}
	...
	public GraphTraversal<Vertex, Vertex> V() {
		tx().readWrite();
		GraphTraversal<Vertex, Vertex> traversal = new DefaultGraphTraversal<Vertex, Vertex>(
				this);
		traversal.addStep(new SparkseeGraphStep<Vertex>(traversal,
				Vertex.class, this));
		return traversal;
	}
	...
	public GraphTraversal<Edge, Edge> E() {
		tx().readWrite();
		GraphTraversal<Edge, Edge> traversal = new DefaultGraphTraversal<Edge, Edge>(
				this);
		traversal.addStep(new SparkseeGraphStep<Edge>(traversal, Edge.class,
				this));
		return traversal;
	}
}
