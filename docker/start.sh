...
if [ -n "$XRAYHOST" ]; then sed  "s/REPLACE_XRAY_HOST/${XRAYHOST}/g" /home/root/sparksee/docker/logback.template.groovy > /usr/local/sparksee/target/gremlin-server-3.0.0-SNAPSHOT-standalone/conf/logback.groovy; fi
